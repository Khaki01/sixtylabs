...
export default function AudioManipulator() {
  ...
  useEffect(() => {
    ...
    if (clipIndex !== null && clipIndex < clips.length) {
      ...
      // No clip assigned, advance after short delay
      setTimeout(() => {
        setCurrentSequencerStep((prev) => (prev + 1) % 16);
      }, 500);
    }
...
//   useEffect(() => {
//     console.log(samplerMode, isPadsPlaying);
//     if (samplerMode === "sequencer") {
//       pauseAudio();
//       if (isPadsPlaying) {
//         console.log("Trying to run sampler");
//         for (const clip of clips) {
//           playAudio(clip);
//         }
//       }
//     }
//   }, [samplerMode, isPadsPlaying]);

//   const asyncPlayAudio = async (clip?: Clip) => {
//     if (!processedBuffer || !audioContextRef.current) return;
//     const clipToPlay = clip ?? clips.find((c) => c.id === selectedClipId);
...
//                 isPlaying={isPlaying}
//                 setSamplerMode={setSamplerMode}
//                 setIsPadsPlaying={setIsPadsPlaying}
//                 onPlayStateChange={(playing) => {
//                   // if (playing) {
//                   //   playAudio();
//                   // } else {
