...
export default function WaveformVisualizer({
  ...
  const [draggedClipEdge, setDraggedClipEdge] = useState<{ clipId: string; edge: "start" | "end" } | null>(null)
  const [hoveredClipEdge, setHoveredClipEdge] = useState<{ clipId: string; edge: "start" | "end" } | null>(null)
  ...
  const drawWaveform = useCallback(() => {
    ...
    clips.forEach((clip, index) => {
      if (clip.endTime < visibleStartTime || clip.startTime > visibleEndTime) return

      const clipStartX = Math.max(0, ((clip.startTime - visibleStartTime) / visibleDuration) * width)
      const clipEndX = Math.min(width, ((clip.endTime - visibleStartTime) / visibleDuration) * width)

      // Light gray fill
      ctx.fillStyle = "rgba(128, 128, 128, 0.15)"
      ctx.fillRect(clipStartX, 0, clipEndX - clipStartX, height)

      const foregroundColor = getComputedStyle(canvas).getPropertyValue("--color-foreground") || "#262626"

      const isStartHovered = hoveredClipEdge?.clipId === clip.id && hoveredClipEdge?.edge === "start"
      const isStartDragged = draggedClipEdge?.clipId === clip.id && draggedClipEdge?.edge === "start"

      ctx.strokeStyle = isStartHovered || isStartDragged ? foregroundColor : foregroundColor
      ...
      const isEndHovered = hoveredClipEdge?.clipId === clip.id && hoveredClipEdge?.edge === "end"
      const isEndDragged = draggedClipEdge?.clipId === clip.id && draggedClipEdge?.edge === "end"

      ctx.strokeStyle = isEndHovered || isEndDragged ? foregroundColor : foregroundColor
    ...
    if (isCreatingClip && clipStartTime !== null && currentClipEndTime !== null) {
      const tempClipStartX = Math.max(0, ((clipStartTime - visibleStartTime) / visibleDuration) * width)
      const tempClipEndX = Math.min(width, ((currentClipEndTime - visibleStartTime) / visibleDuration) * width)
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        const timeAtMouse = visibleStartTime + mousePercentage * visibleDuration
        ...
        if (newZoom > 1) {
          ...
          const newVisibleStartTime = timeAtMouse - mousePercentage * newVisibleDuration
  ...
  const getClipEdgeAtPosition = (x: number): { clipId: string; edge: "start" | "end" } | null => {
    ...
    for (const clip of clips) {
      if (clip.endTime < visibleStartTime || clip.startTime > visibleEndTime) continue

      const clipStartX = Math.max(0, ((clip.startTime - visibleStartTime) / visibleDuration) * rect.width)
      const clipEndX = Math.min(rect.width, ((clip.endTime - visibleStartTime) / visibleDuration) * rect.width)
      ...
      const updatedClips = clips.map((clip) => {
        if (clip.id === draggedClipEdge.clipId) {
          if (draggedClipEdge.edge === "start") {
            ...
            return { ...clip, startTime: Math.min(newTime, clip.endTime - 0.1) }
          } else {
            // Ensure end doesn't go before start (leave at least 0.1s)
            return { ...clip, endTime: Math.max(newTime, clip.startTime + 0.1) }
          }
        }
        return clip
      })
  ...
  const handleMouseUp = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (isCreatingClip && clipStartTime !== null && currentClipEndTime !== null && onClipsChange) {
